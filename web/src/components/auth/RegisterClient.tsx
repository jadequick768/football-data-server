...
export default function RegisterClient() {
  ...
  return (
    <main style={{ maxWidth: 420, margin: '0 auto', padding: 16 }}>
      <h1 style={{ margin: '10px 0 16px', fontSize: 20 }}>Register</h1>
      ...
      <form onSubmit={submit} style={{ background: '#111827', border: '1px solid #1F2937', borderRadius: 14, padding: 12, display: 'grid', gap: 10 }}>
        <label style={{ display: 'grid', gap: 6 }}>
          <span style={{ color: '#9CA3AF', fontSize: 12 }}>Display name</span>
          <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} style={{ padding: 10, borderRadius: 10, border: '1px solid #1F2937', background: '#0B0F14', color: '#fff' }} />
        </label>
        ...
        <div style={{ fontSize: 12, color: '#9CA3AF' }}>
          Have an account?{' '}
          <Link href={`/auth/login?next=${encodeURIComponent(next)}`} style={{ color: '#F5C400', textDecoration: 'none', fontWeight: 800 }}>
            Login
          </Link>
        </div>
      </form>
